...
//Our goal is to determine which games are possible with only 12 red cubes, 13 green cubes, and 14 blue cubes, and then add the ID together of those games.
const red = 12;
const green = 13;
const blue = 14;
//My idea here is to first split off the word Game and the : - so that we can then separate the games into the 3 rounds.
input = input.split("\n")//Split into rows
input = input.map(row => {
    return row.split(": ")[1].split(";");//Split off the Game: xxx and then split into rounds
})
for(let i = 0; i<input.length;i++){//iterate each game (input[i])
    //If the game is invalid, I want to replace it with null.
    let thisGameValid = true;
    for(let r = 0;r<input[i].length;r++){//iterate each round. (input[i][r])
        input[i][r] = input[i][r].split(",")
        //Iterate each color in each round. (input[i][r][c])
        for(let c = 0; c<input[i][r].length;c++){

            if(input[i][r][c].includes("red")){
                input[i][r][c] = parseInt(input[i][r][c].split(" red"))
                if(input[i][r][c] > red){
                    thisGameValid = false
                }
            }

            else if(input[i][r][c].includes("blue")){
                input[i][r][c] = parseInt(input[i][r][c].split(" blue"))
                if(input[i][r][c] > blue){
                    thisGameValid = false
                }
            }

            else if(input[i][r][c].includes("green")){
                input[i][r][c] = parseInt(input[i][r][c].split(" green"))
                if(input[i][r][c] > green){
                    thisGameValid = false
                }
            }
        }
...
//Now we have nullified invalid games. We can grab the id of entries that arent null, and add them together.
let totalOfIDs = 0;
input.map(entry => {
    if(entry!==null){
        totalOfIDs+=(input.indexOf(entry)+1)
    }
})
