...
const splitInput = input.split("\n").map(input => {
    //I think the easiest way to do this is to remove all non-digit characters.

    //cast string to array and remove letters.
    const newInput = input.split("").filter(char => {
        if(parseInt(char, 10).toString() !== "NaN"){
            return char
        }
    });
...
//now that these values are numbers, we can add the first and the last value in each input together.
const FLValues = splitInput.map(input => {
    return parseInt(input[0] + input[input.length - 1])
})
//should be able to reduce and get final answer. Hopefully.
console.log(FLValues.reduce((acc, cur) => acc + cur))