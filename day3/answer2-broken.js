...
//This time we are looking for "gears" - when 2 numbers touch a *. Then we need to take the two touching a * and multiply them together and add all of those up.
const gearRatios = [];
for(let i=0; i<input.length;i++){
    for(let r=0; r<input[i].length; r++){
        //Whenever we find a *, we need to look for numbers immediately adjacent. Then look left and right (up to two times potentially) and find the numbers.
        //Gear ratio can be above, below, or on the same row.

        //Find a gear
        if(input[i][r] === "*"){
            let firstFoundNumber;
            let secondFoundNumber;

            for(let s = r-1; s<r+2; s++){
                //Now we check above the gear for any numbers.
                if(input[i-1]){
                    if(parseInt(input[i-1][s]).toString() !== "NaN"){//If we find a number
                        //Set up the number (still as a string so we can concat)
                        let num = input[i-1][s]
                        //Check Left of num
                        if(parseInt(input[i-1][s-1]).toString() !== "NaN" && input[i-1][s-1] !== undefined){
                            num = input[i-1][s-1] + num

                            //Check one more to the left.
                            if(parseInt(input[i-1][s-2]).toString() !== "NaN" && input[i-1][s-2] !== undefined){
                                num = input[i-1][s-2] + num
                            }
                        }
                        //Check Right of num
                        if(parseInt(input[i-1][s+1]).toString() !== "NaN" && input[i-1][s+1] !== undefined){
                            num = num + input[i-1][s+1]

                            //Check one more to the right.
                            if(parseInt(input[i-1][s+2]).toString() !== "NaN" && input[i-1][s+2] !== undefined){
                                num = num + input[i-1][s+2]
                            }
                        }
                        firstFoundNumber = parseInt(num)
                    }
                }
                //Now we check below the gear for any numbers.
                if(input[i+1]){
                    if(parseInt(input[i+1][s]).toString() !== "NaN"){//If we find a number
                        //Set up the number (still as a string so we can concat)
                        let num = input[i+1][s]
                        ...
                        if(firstFoundNumber){
                            secondFoundNumber = parseInt(num)
                        }
                        else{
                            firstFoundNumber = parseInt(num);
                        }
            ...
            //Check left of the gear for numbers.
            if(input[i][r-1]){
                //immediate left
                if(parseInt(input[i][r-1]).toString() !== "NaN"){
                    let num = input[i][r-1]
                    //one more left
                    if(input[i][r-2]){
                        if(parseInt(input[i][r-2]).toString() !== "NaN"){
                            num = input[i][r-2] + num;
                            //last check
                            if(input[i][r-3]){
                                if(parseInt(input[i][r-3]).toString() !== "NaN"){
                                    num = input[i][r-3] + num
                                }
                            }
                        }
            ...
            //Check right of the gear for numbers.
            if(input[i][r+1]){
                //immediate right
                if(parseInt(input[i][r+1]).toString() !== "NaN"){
                    let num = input[i][r+1]
            ...
            if(firstFoundNumber && secondFoundNumber){
                gearRatios.push(firstFoundNumber * secondFoundNumber);
            }
        }
    }
}
console.log(gearRatios.reduce((acc, cur) => acc += cur))