...
//Unlike last time, we are finding the MINIMUM number of cubes it would take to make each one possible. No worrying about if it was or wasnt possible.
//Then we take the multiplied answer for each one and add those all together.
input = input.split("\n")//Split into rows
input = input.map(row => {
    return row.split(": ")[1].split(";");//Split off the Game: xxx and then split into rounds
})
//Now we want to iterate over each game.
for(let i=0;i<input.length;i++){

    //Define minimums of each color
    let minRed = 0;
    let minBlue = 0;
    let minGreen = 0;

    for(let r=0;r<input[i].length;r++){//Iterate over each round
        input[i][r] = input[i][r].split(",");//Split each round by color.

        for(let c=0; c<input[i][r].length;c++){//Iterate over each color now.
            if(input[i][r][c].includes("red")){
                input[i][r][c] = parseInt(input[i][r][c].split(" red"));
                if(input[i][r][c] > minRed){
                    minRed = input[i][r][c];
                }
            }
                ...
                if(input[i][r][c] > minGreen){
                    minGreen = input[i][r][c];
                }
            }
        }
    }
    input[i] = minRed * minBlue * minGreen//Replace the game with the multiplied minimums.
}
