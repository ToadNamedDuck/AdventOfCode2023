...
let inputArrays = input.map(row => {
    return row.split("");
})

//Now we have 140 equal-length arrays.

//We are supposed to add ALL values which are a part number together, so we need a way to store part numbers.
const partNumbers = [];

//Begin parsing through the entire table.
for(let i = 0; i<inputArrays.length;i++){

    //Begin to parse through each row entry.
    for(let r = 0;r<inputArrays[i].length;r++){
        let potentialPartNumber = ""
        let isRealPartNumber = false;
        //Check to see if the current member is a number digit AND NOT a period.
        if(parseInt(inputArrays[i][r]).toString() !== "NaN"){
            potentialPartNumber = inputArrays[i][r]
            //If the entry IS a number, first check the next two spaces (since all of the numbers are 3 digits) and concat them
            if(parseInt(inputArrays[i][r+1]).toString() !== "NaN"){
                potentialPartNumber += inputArrays[i][r+1]
            }
            //If there is now a two digit number, check for three digits.
            if(potentialPartNumber.length === 2){
                if(parseInt(inputArrays[i][r+2]).toString() !== "NaN"){
                    potentialPartNumber += inputArrays[i][r+2]
                }
            }
            //Now we have our potential part number, and it's time to check the indicies of the several "surrounding" spaces.
            for(let sur=r-1; sur<(r+potentialPartNumber.length+1); sur++){
                //Check above
                if(inputArrays[i-1]){
                    if(inputArrays[i-1][sur] !== "." && inputArrays[i-1][sur] !== undefined){
            ...
            //Check before.
            if(inputArrays[i][r-1] !== "." && inputArrays[i][r-1] !== undefined){
                isRealPartNumber = true;
            }
            //Check after.
            if(inputArrays[i][r+potentialPartNumber.length] !== "." && inputArrays[i][r+potentialPartNumber.length] !== undefined){
                isRealPartNumber = true;
            }
                            //Special case that the spot previous is a number
                            if(parseInt(inputArrays[i][r-1]).toString() !== "NaN"){
                                isRealPartNumber = false;
                            }
        }
        if(isRealPartNumber){
            partNumbers.push(parseInt(potentialPartNumber))
        }
    }
}
